...
export default function EditFeaturedPage() {
  ...
  async function handleMainImageChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const url = await uploadImageToS3(file, productId, token);
      setMainImage(url);
    } catch (err) {
  ...
  async function handleGalleryImageChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    for (let i = 0; i < toUpload.length; i++) {
      try {
        ...
        const token = await getToken();
        const url = await uploadImageToS3(toUpload[i], productId, token);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!mainImage) {
      showToast('El producto debe tener una imagen principal', 'error');
      return;
    }

    try {
      setSubmitting(true);
      await updateFeaturedProduct(productId, {
        ...result.data,
        image: mainImage,
        images: galleryImages.length > 0 ? galleryImages : undefined,
      });
      showToast('Producto actualizado correctamente');
      setTimeout(() => router.push('/admin/featured'), 1200);
    } catch (err) {
  ...
  async function handleDelete() {
    if (!confirm(`¿Desactivar este producto destacado?`)) return;
    try {
      setDeleting(true);
      ...
      showToast(err instanceof Error ? err.message : 'Error al desactivar el producto', 'error');
    } finally {
      setDeleting(false);
    }
  }
  ...
  if (notFound) {
    return (
      <div className="text-center py-20">
        <p className="text-neutral-400 font-medium mb-4">Producto no encontrado.</p>
        <Button variant="outline" size="sm" onClick={() => router.push('/admin/featured')}>
          Volver a la lista
        </Button>
      </div>
    );
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-2xl font-bold text-primary">Editar Producto Destacado</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-2xl shadow-sm p-6 space-y-4">
              ...
              {uploading === 'main' ? (
                ...
                <div className="relative w-full aspect-square max-h-[200px]">
                  <Image
                    ...
                    sizes="200px"
                    unoptimized={mainImage.startsWith('https://')}
                  />
                </div>
              ) : (
                <div className="text-center text-neutral-400">
                  <div className="text-3xl mb-2">📷</div>
                  <p className="text-xs font-medium">Click para cambiar imagen</p>
                </div>
              )}
            </div>
            <input
            ...
            />
            {mainImage && (
              <p className="text-xs text-neutral-400 truncate" title={mainImage}>
                {mainImage.split('/').pop()}
              </p>
            )}
          </div>
