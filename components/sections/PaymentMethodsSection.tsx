          ...
          {PAYMENT_METHODS.map((method, index) => (
            <div
              ...
              className={cn(
                ...
                'hover:border-[#f4c430] hover:shadow-xl',
                'transition-all duration-300 hover:-translate-y-2',
                'group'
              )}
            >
              {/* Decorative background */}
              <div className="absolute inset-0 bg-gradient-to-br from-[#f4c430]/5 to-transparent opacity-0 group-hover:opacity-100 rounded-xl sm:rounded-2xl transition-opacity duration-300" />

              {/* Icon */}
              <div className="relative z-10 mb-6 inline-flex items-center justify-center w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-gradient-to-br from-[#f4c430]/20 to-[#f4c430]/5 text-[#f4c430] group-hover:bg-[#f4c430] group-hover:text-white transition-all duration-300">
                {method.icon}
              </div>

              {/* Content */}
              <div className="relative z-10">
                <h3 className="text-xl sm:text-2xl font-bold text-[#1a3a52] mb-2">
                  {method.title}
                </h3>
                ...
                <ul className="space-y-3">
                  {method.details.map((detail, i) => (
                    <li key={i} className="flex items-center text-neutral-700">
                      <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-[#f4c430] text-[#1a3a52] text-xs font-bold mr-3 flex-shrink-0">
                        ✓
                      </span>
              ...
              {/* Bottom accent */}
              <div className="absolute bottom-0 left-0 w-0 h-1 bg-gradient-to-r from-[#f4c430] to-transparent group-hover:w-full transition-all duration-300 rounded-b-xl sm:rounded-b-2xl" />
            </div>
          ))}
        </div>
